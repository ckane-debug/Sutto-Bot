#Mr.D's example robot line follower + castle crasher code
...
down_eye = ColorSensor(Port.E)

while True:
    #if (down_eye.color() != Color.YELLOW):
        #left_motor.run(-100)
        #right_motor.run(100)
        #wait(50)
    #else:
    ...
    if down_eye.color() == Color.WHITE:
        ...
        right_motor.run(100)
        left_motor.run(-100)
        left_motor.run_angle(100, 350)
        wait(75)
    else:
        if eyes.distance() < 400:
            left_motor.stop()
            right_motor.stop()
            wait(100)
            left_motor.run(-100)
